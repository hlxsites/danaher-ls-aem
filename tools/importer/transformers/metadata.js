...
const addArticleMeta = (document, meta) => {
  ...
  if (articleinfo) {
    ...
    if (articleinfoEL) {
      if (articleinfoEL.hasAttribute("articlename"))
        meta.authorName = articleinfoEL.getAttribute("articlename");
      if (articleinfoEL.hasAttribute("title"))
        meta.authorTitle = articleinfoEL.getAttribute("title");
      if (articleinfoEL.hasAttribute("postdate"))
        meta.publishDate = new Date(
          Date.parse(`${articleinfoEL.getAttribute("postdate")} UTC`)
        ).toUTCString();
      ...
      if (articleinfoEL.hasAttribute("opco"))
        meta.brand = articleinfoEL.getAttribute("opco");
...
const addDataLayerMeta = (document, html, meta) => {
  ...
  try {
    const dataLayerJson = filteredScripts[0]
      ? JSON.parse(
    ...
    if (dataLayerJson) {
      meta.creationDate = dataLayerJson[1]
        ? new Date(
            Date.parse(`${dataLayerJson[1]?.page.creationDate} UTC`)
          ).toUTCString()
        : "";
      meta.updateDate = dataLayerJson[1]
        ? new Date(
            Date.parse(`${dataLayerJson[1]?.page.updateDate} UTC`)
          ).toUTCString()
        : "";
    }
  } catch (error) {
...
const addSolutionMeta = (url, meta) => {
  ...
  if (url.pathname.match(/^\/content\/danaher\/ls\/us\/en\/solutions\//)) {
    const solution = url.pathname
      .replace(/^\/content\/danaher\/ls\/us\/en\/solutions\//, "")
      .replace(/\.html$/, "")
      .split("/");
...
const addCategoryMeta = (url, meta) => {
  ...
  if (
    url.pathname.match(
      /^\/content\/danaher\/ls\/us\/en\/products\/(?!family\/|sku\/)/
    )
  ) {
...
const isDefaultProductPage = (url) =>
  url.pathname.match(/\/content\/danaher\/ls\/us\/en\/products\/product-coveo/);
...
const addSKUMeta = (url, meta) => {
  ...
  if (
    url.pathname.match(
      /\/content\/danaher\/ls\/us\/en\/products\/(family\/|sku\/|bundle\/)/
    )
  ) {
    const sku = url.pathname
      .replace(/^\/content\/danaher\/ls\/us\/en\/products\//, "")
      .replace(/\.html$/, "")
      .split("/");
    meta.sku = sku.pop();
...
const createMetadata = (main, document, html, params, urlStr) => {
  ...
  console.log(" metadata.js 86 : ", url);
  ...
  if (canonical) {
    ...
    if (
      ...
      canonicalUrl = new URL(href.pathname, "https://lifesciences.danaher.com/")
        .href;
    }
  ...
  if (pageorder && pageorder.content) {
    meta.pageorder = pageorder.content;
  }
