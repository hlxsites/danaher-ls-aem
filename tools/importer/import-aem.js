...
function transformColumns(main, document) {
  ...
  // Find the .columns-wrapper inside main
  const columnsWrapper = main.querySelector('.columns-wrapper');
  if (!columnsWrapper) return;

  // Find the .columns.block.columns-2-cols inside the wrapper
  const columnsBlock = columnsWrapper.querySelector('.columns.block.columns-2-cols');
  if (!columnsBlock) return;

  // Find the first .columns-img-col containing the <picture> element
  const imgCol = columnsBlock.querySelector('.columns-img-col picture');
  if (!imgCol) return;

  // Clone the picture element to avoid manipulating the original DOM
  const pictureClone = imgCol.cloneNode(true);

  // Create a container div to wrap the image block
  const imageBlock = document.createElement('div');
  imageBlock.className = 'image-block-wrapper';

  // Create a label for the image block
  const label = document.createElement('p');
  label.className = 'image-block-label';
  label.textContent = 'Image Block';

  // Append label and picture to the image block container
  imageBlock.appendChild(label);
  imageBlock.appendChild(pictureClone);

  // Insert the image block before the columnsWrapper
  columnsWrapper.parentNode.insertBefore(imageBlock, columnsWrapper);

  // Optionally remove the original columnsWrapper if only the image block is needed
...
const transformCardList = (main, document) => {
  main.querySelectorAll('div.card-list.block').forEach((cardListBlock) => {
    const cards = cardListBlock.querySelectorAll('ul > li');

    if (!cards.length) return;

    // Create a container ul with your grid classes
    ...
    // For each card, create a list item with image and content
    cards.forEach((card) => {
      const link = card.querySelector('a');
      const img = card.querySelector('img');
      const title = card.querySelector('h5')?.textContent.trim() || '';
      const desc = card.querySelector('p')?.textContent.trim() || '';
      const meta = card.querySelector('small')?.textContent.trim() || ''; // example for meta info
      ...
      if (img) {
        ...
        imgClone.className = 'w-full object-cover'; // add styling classes as needed
        cardInner.appendChild(imgClone);
      }
      ...
      if (link && title) {
        ...
        cardInner.appendChild(titleLink);
      }
      ...
      if (meta) {
        ...
        cardInner.appendChild(metaPara);
      }
      ...
      if (desc) {
        ...
        cardInner.appendChild(descPara);
      }
    ...
    // Clear original block content and append the new card list
