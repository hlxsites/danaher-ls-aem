...
export function buildArticleSchema() {
  const data = {
    ...
    "@type": "Article",
    "@id": `https://lifesciences.danaher.com${makePublicUrl(
  ...
  if (getMetadata("creationdate"))
    data.datePublished = getMetadata("creationdate");
...
export function buildProductSchema() {
  const data = {
    ...
    offers: {
      ...
      url: `https://lifesciences.danaher.com${makePublicUrl(
        window.location.pathname
      )}`,
      seller: {
        "@type": "Organization",
...
export function buildItemListSchema(srcObj, type) {
  ...
  if (productType.includes(type))
    ...
    name = `${document.querySelector("h1").textContent} Process Steps`;
  else if (type === "solution-products-steps")
