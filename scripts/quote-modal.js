import { button, div, label, p, span, textarea } from './dom-builder.js';
  ...
  return div(
    div(
      ...
      label(
        ...
        'Describe your problem or desired solution to add to your quote cart and one of our experts will assist in find the best solution for you'
      )
    ),
    div(
      { class: 'mt-3' },
      textarea({
        class:
          'quote-textarea block w-full px-1 py-2 border border-gray-300 rounded-md shadow-sm focus:border-gray-300 focus:ring-gray-300 sm:text-sm',
        name: 'quote',
        rows: '4',
      })
    ),
    div(
      ...
      button(
        {
          class:
            'p-2 text-sm text-danaherpurple-500 bg-white border-2 border-danaherpurple-500 hover:text-white hover:bg-danaherpurple-800 rounded-full',
          name: 'continue',
        },
      ...
      button(
        {
          class: 'py-2 text-sm btn btn-primary-purple rounded-full',
          name: 'submit',
        },
    ...
    div(
      ...
      div(
        ...
        p(
          ...
          'Please enter your problem or desired solution.'
        )
      )
    ),
    div(
      {
        class:
          'flex flex-col p-4 mt-4 rounded-md bg-danaherlightblue-500 bg-opacity-10',
      },
      p({ class: 'text-xs font-medium text-gray-700 m-0' }, 'Quote Tip.'),
      p(
        { class: 'font-sans text-xs font-normal text-gray-700' },
        'Be as detailed as possible so we can best serve your request.'
      )
    )
  );
}
...
export function addRequestforQuote(dialogElement, gotoQuoteCart = false) {
  ...
  const image = document
    .getElementsByClassName('image-content')
    ?.item(0)
