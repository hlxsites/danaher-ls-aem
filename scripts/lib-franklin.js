function wrapTextNodes(block) {
  ...
  const wrap = (el) => {
    ...
    [...el.attributes]
      ...
      .forEach(({ nodeName: nodeName, nodeValue: nodeValue }) => {
...
export function sampleRUM(checkpoint, data = {}) {
  ...
  const defer = (fnname) => {
    sampleRUM[fnname] =
      sampleRUM[fnname] ||
      ((...args) => sampleRUM.defer.push({ fnname: fnname, args: args }));
  };
  sampleRUM.drain =
    sampleRUM.drain ||
    ((dfnname, fn) => {
      ...
      sampleRUM.defer
        .filter(({ fnname: fnname }) => dfnname === fnname)
        .forEach(({ fnname: fnname, args: args }) =>
  ...
  try {
    ...
    if (!window.hlx.rum) {
      ...
      const weight = usp.get("rum") === "on" ? 1 : 20;
...
export function getMetadata(name) {
  ...
  const meta = [...document.head.querySelectorAll(`meta[${attr}="${name}" i]`)]
    .map((m) => m.content)
    .join(", ");
  return meta || "";
}
      ...
      if (!ICONS_CACHE[iconName]) {
        ...
        try {
          ...
          if (iconName.startsWith("dam-")) {
            ...
            iconSource += `/content/dam/danaher/system/icons/${iconName
              .substring(4)
              .replace("_", " ")}.svg`;
          }
...
function getBlockConfig(block) {
  const { blockName: blockName } = block.dataset;
  const jsPath = blockName
    ? `${window.hlx.codeBasePath}/blocks/${blockName}/${blockName}.js`
    : "";
  const cssPath = blockName
    ? `${window.hlx.codeBasePath}/blocks/${blockName}/${blockName}.css`
    : "";
  const original = { blockName: blockName, jsPath: jsPath };
