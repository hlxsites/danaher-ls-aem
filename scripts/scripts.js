...
const LCP_BLOCKS = ['breadcrumb', 'product-hero', 'carousel', 'columns'];
const TEMPLATE_LIST = {
  blog: {
    templateName: 'blog',
    dependencies: [
...
export function moveAttributes(from, to, attributes) {
  if (!attributes) {
    attributes = [...from.attributes].map(({ nodeName: nodeName }) => nodeName);
  }
...
export function moveInstrumentation(from, to) {
  moveAttributes(
    ...
    to,
    [...from.attributes]
      .map(({ nodeName: nodeName }) => nodeName)
      .filter(
        (attr) =>
...
window.dataLayer.push({ page: getDLPage() });
