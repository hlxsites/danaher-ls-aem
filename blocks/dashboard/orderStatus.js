...
import { renderPagination } from "./dashboardutils.js";
...
export const dynamicTableContent = async (orderDetailsResponse) => {
  ...
  const tableRow = (order, po, status, creationDate, orderTotal) => {
    ...
    let formattedAmount = parseFloat(orderTotal).toLocaleString('en-US', {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2
    });
    let formattedWords = status
      .toLowerCase()
...
export const orderStatus = async () => {
  ...
  const status = ["Status", "All", "Approved", "Cancelled", "Invoiced", "Shipped", "Submitted", "New"]
  ...
  const dropDown = (days) => {
    ...
    days.forEach((day, index) => {
      ...
      option.value = day.toLowerCase().replace(/\s+/g, "-"); // e.g., "Last 7 days" -> "last-7-days"
    ...
    const searchElement = div(
      ...
      div(
        ...
        input({
          ...
          class:
            "searchbox min-w-[415px] h-10 rounded-md pl-9 text-base block px-8 py-5 text-gray-600 font-extralight  border-t border-l border-r border-b border-gray-300 ",
          "aria-label": dtName,
        }),
        div(
          {
  ...
  const downloadSvg = div(
    ...
    span({
      class: `icon icon-Download [&_svg>use]:stroke-danaherpurple-500 !w-[60px] !h-[60px] p-[18px] [&_svg>use]:hover:stroke-danaherpurple-800`,
    }),
    div(
      {
        class:
  ...
  const orderDesc = div(
    ...
    div(
      {
        class:
          "w-28 pl-2 pr-2.5 py-1.5 rounded shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] flex justify-center items-center gap-2 overflow-hidden",
      },
      downloadSvg
    )
  );
