    ...
    div(
      ...
      div(
        ...
        div(
          ...
          p({
            class:
              "w-3/4 h-4 bg-danaheratomicgrey-200 rounded [&:not(:first-child)]:opacity-40",
          }),
          p({
            class:
              "w-2/5 h-3 bg-danaheratomicgrey-200 rounded [&:not(:first-child)]:opacity-20",
          }),
          p({
            class:
              "w-4/5 h-5 bg-danaheratomicgrey-200 rounded [&:not(:first-child):even]:opacity-40",
...
function iterateChildren(filter, node) {
  ...
  const liEl = li(
    ...
    button(
      ...
      span(
        { part: "value-label", class: "text-sm truncate w-[15rem] block" },
        node?.value
      ),
...
function buildAllCategories() {
  return li(
    ...
    button(
      {
        class:
          "p-1 text-left hover:bg-gray-100 flex flex-row items-center gap-2",
        "aria-pressed": true,
        onclick: clearFilter,
      },
      ...
      span(
        {
          part: "value-label",
          class: "value-label peer-hover:text-error text-sm",
        },
        "All Categories"
      )
    )
...
function addFacetFilters(filter, fecetList) {
  ...
  if (selectedFacet && filter.facetId === "workflowname") {
    ...
    allCategories.append(
      ul({ part: "values", class: "parents m-1 w-full" }, selectedFacet)
    );
  ...
  opcoEl?.forEach((el) => {
    const iconClass =
      el?.getAttribute("aria-pressed") === "false"
        ? "icon icon-square pr-2"
        : "icon icon-check-square pr-2";
    el.insertBefore(span({ class: iconClass }), el.firstChild);
  });

  const selectedButton = fieldUL.querySelectorAll(
    "li.child > button.workflowname:not(.active)"
  );
  if (selectedButton.length > 0) {
    selectedButton.forEach((buttonEl) => {
      buttonEl.insertBefore(
        span({ class: "icon icon-chevron-left pr-2" }),
        buttonEl.firstChild
      );
    });
...
function addSearch(facetsObj) {
  ...
  facetsObj.querySelector(".btn-text-transparent").after(
    div(
      ...
      div(
        ...
        input({
          part: "search-input",
          class:
            "input-primary w-full h-full px-9 placeholder-neutral-dark text-sm group border border-neutral rounded-lg",
          type: "text",
          placeholder: "Search",
        ...
        div(
          {
            class:
              "search-icon pointer-events-none absolute inline-flex justify-center items-center left-0 w-9 h-full text-on-background",
          },
...
function addFacetHeading(facetsObj, name) {
  facetsObj.append(
    button(
      {
        class:
          "btn-text-transparent flex font-bold justify-between w-full py-1 px-2 text-lg rounded-none",
  ...
  facetsObj.append(fieldset({ class: "contents all-facet-list" }));
}
    ...
    return parent.insertBefore(
      li(
        ...
        button(
          {
            class:
              "btn-outline-secondary rounded-full !border-gray-300 px-2 py-1 text-sm truncate w-64 block",
            part: "breadcrumb-button",
            "aria-pressed": true,
            onclick: clearFilter,
            title: `Process Step : ${[...workflowName].join(" / ")}`,
            "aria-label": `Remove inclusion filter on Process Step: ${[
              ...workflowName,
            ].join(" / ")}`,
          },
...
function resultList(response, categoryDiv) {
  const breadcrumbFilter = div(
    ...
    div(
      ...
      ul(
        {
          class:
            "breadcrumb-list flex gap-1 flex-col md:flex-row absolute w-full",
        },
        li(
          button(
            {
              class:
                "btn-outline-secondary rounded-full !border-gray-300 px-2 py-1",
              "aria-pressed": true,
              onclick: (e) => {
                clearFilter(e, true, true);
              },
              part: "clear",
            ...
            span("Clear"),
            span({
              class: "icon icon-close w-4 h-4 align-middle !fill-current",
            })
          )
        )
      )
  ...
  if (workflowName.size > 0 || opco.size > 0)
    categoryDiv.append(breadcrumbFilter);

  categoryDiv.append(
    div(
      { class: "list-wrapper display-grid density-compact image-small mt-6" },
      div({
        class: "result-list grid grid-cols-1 lg:grid-cols-3 gap-6",
        part: "result-list",
      })
    )
  );
  ...
  response.results.forEach((product) => {
    const image = product?.raw?.images
      ? `${product?.raw?.images[0]}?$danaher-mobile$&fmt=webp&wid=300`
      : "";
      ...
      a(
        ...
        div(
          ...
          div(
            {
              class:
                "relative w-full h-full flex flex-col border rounded-md cursor-pointer transition z-10",
            },
            div(
              img({
            ...
            div(
              a(
                {
                  class:
                    "!px-7 !text-lg !font-semibold !text-danahergray-900 !line-clamp-3 !break-words !h-14",
              ...
              div(
                {
                  class:
                    "description !px-7 mb-4 text-sm text-gray-900 break-words line-clamp-4 !h-20 py-4",
                },
            ...
            div(
              {
                class:
                  "inline-flex items-center w-full px-6 py-5 space-x-4 bg-gray-100",
              },
              span(
                {
...
function getQueryString(buttonEl) {
  ...
  if (workflowName.size > 0)
    queryMap.set("workflowname", [...workflowName].join(","));
