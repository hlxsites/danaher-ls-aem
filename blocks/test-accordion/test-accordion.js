...
function toggleAccordion(blockUUID, activeAccordion) {
  const allAccordions = document.querySelectorAll(`div#accordion-${blockUUID} div.test-item`);
...
function createAccordionBlock(question, answer, image, uuid, parentElement, index, customUUID) {
  ...
  parentElement.classList.add('test-item', 'relative', 'py-6', 'border-t', 'border-gray-300');
...
export default async function decorate(block) {
  document
    .querySelector('.tes-accordion-wrapper')
    ?.parentElement?.removeAttribute('class');
  document
    .querySelector('.test-accordion-wrapper')
    ?.parentElement?.removeAttribute('style');
  const accordionContainerWrapper = div({
    class: 'dhls-container mx-auto flex flex-col md:flex-row gap-6 px-5 md:px-0',
  });
  const accordionContainerTitle = block.querySelector('[data-aue-prop="test-accordion_title"]')?.textContent.trim() || '';

  const customUUID = generateUUID();

  const dynamicData = [...block.querySelectorAll('[data-aue-model="test-item"]')].map((element) => {
  ...
  const filteredDynamicData = dynamicData.filter((item) => item.question && item.answer);
