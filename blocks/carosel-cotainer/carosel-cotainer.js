...
export default function decorate(block) {
  ...
  const rightBannerBlocks = children.filter(
    (child) => child.dataset.aueModel === "main-banner"
  );

  // --- LEFT SIDE ---
  const left = div({
    class: "carousel-left flex flex-col justify-center max-w-xl p-4",
  });
  ...
  leftContentDivs.forEach((el) => {
    ...
    const linkP = [...clone.querySelectorAll("p")].find((p) =>
    ...
    if (linkP) {
      ...
      link.className =
        "inline-block mt-4 text-danaherpurple-600 font-semibold hover:underline";
  ...
  const carouselWrapper = div({
    class: "carousel-right relative w-full overflow-hidden flex-1",
  });
  const track = div({
  ...
  rightBannerBlocks.forEach((banner) => {
    const slide = div({
      class:
        "carousel-slide w-full shrink-0 flex flex-col items-center text-center px-4",
    });

    const brandTitle = banner.querySelector('[data-aue-prop="brand_title"]');
    const brandDesc = banner.querySelector(
      '[data-aue-prop="brand_description"]'
    );
    ...
    if (img) {
      ...
      picture.querySelector("img").className =
        "max-h-60 w-auto mx-auto mb-4 rounded";
  ...
  const btnLeft = button(
    {
      class:
        "carousel-prev absolute left-4 top-1/2 -translate-y-1/2 bg-danahergray-200 rounded-full w-10 h-10 flex items-center justify-center hover:bg-danaherpurple-200",
    },
    "‹"
  );
  const btnRight = button(
    {
      class:
        "carousel-next absolute right-4 top-1/2 -translate-y-1/2 bg-danahergray-200 rounded-full w-10 h-10 flex items-center justify-center hover:bg-danaherpurple-200",
    },
    "›"
  );
