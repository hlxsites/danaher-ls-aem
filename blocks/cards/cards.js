...
export default function decorate(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    if (heading)
      heading.className =
        'card-title text-gray-900 my-2 font-extrabold text-3xl py-2';
    ...
    cardWrapper.className =
      'card-wrapper flex flex-col col-span-1 mx-auto justify-center max-w-xl overflow-hidden pl-8 pr-2 border-l-[0.5px] border-gray-300 transform transition duration-500 hover:scale-105';
    ...
    [...row.children].forEach((elem) => {
      ...
      if (elem?.querySelector('h3'))
        elem.querySelector('h3').className = '!line-clamp-2 !h-16';
      if (elem?.querySelector('h3') && !block.classList.contains('opco'))
        elem.querySelector('h3').className =
          'pl-2 text-lg font-semibold text-danahergray-900 !line-clamp-3 !break-words !h-24';
      if (elem?.querySelector('p'))
        elem.querySelector('p').className =
          'mb-4 text-sm !h-20 !line-clamp-4 !break-words';
      if (elem?.querySelector('p') && !block.classList.contains('opco'))
        elem.querySelector('p').className =
