...
import {   ul, a, p, div, span, h4, li, h3, } from '../../scripts/dom-builder.js';
...
export default async function decorate(block) {
    ...
    if (response?.results.length > 0) {
        ...
        //   product.categoriesname = product?.raw?.categoriesname;
          let categoriesName = [];
          ...
          console.log('categoriesName :', product.categoriesName);
          cardList.append(createCard(product, index, index === 0));
        });

        const divEl = document.createElement('div');
        const h2pEl = document.createElement('h2');
        h2pEl.textContent = 'Product Recommendations - frequentViewed';
        divEl.append(h2pEl);
        block.innerHTML = '';
