...
export default async function decorate(block) {
  ...
  const cardList = ul({
    class:
      'container grid w-full dhls-container md:p-0 dhls-mobile-spacing gap-6 grid-cols-1 sm:grid-cols-1 lg:grid-cols-3  justify-items-start',
  });
  ...
  if (articles.length > 0) {
    divEl = div(
      {
        class:
          'flex items-center justify-between mt-12 dhls-container md:p-0 dhls-mobile-spacing border-t border-gray-300 pt-8',
      },
