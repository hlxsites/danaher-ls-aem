  ...
  const carousel = div(
    ...
    ...clonedBlock.map((element, eleIndex) => {
      ...
      if (cardImage && link) {
        ...
        element.classList.add(...'h-full flex flex-col p-4'.split(' '));
        element.querySelector('strong').classList.add(...'text-base text-gray-400 group-hover:font-bold group-hover:underline'.split(' '));
        element.querySelector('p:not(strong) ~ p').classList.add(...'flex-1 mt-2 mb-3 text-xl font-bold text-gray-900 break-words leading-tight tracking-normal line-clamp-4'.split(' '));
        ...
        anchor.append(cardImage, element);
      }
      return anchor;
    }).filter(Boolean),
  );
