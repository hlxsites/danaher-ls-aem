...
export default async function decorate(block) {
  ...
  const renderCard = (product, viewType = 'grid') => {
    if (!product) return null;
    const { title, url, image, description, showCart, price, unitMeasure, minQty } = product;

    const cardClass = viewType === 'grid'
      ? 'w-[23.5%] min-w-[23.5%] flex-shrink-0 bg-white border border-gray-300 rounded-lg p-4 flex flex-col h-[485px]'
      : 'w-full bg-white border border-gray-300 rounded-lg p-4 flex flex-col';
    ...
    if (image) {
      card.append(img({
        src: image,
        alt: title,
        class: viewType === 'grid' ? 'w-full h-32 object-contain mb-4' : 'w-[120px] h-[120px] object-contain mb-4'
      }));
    }
  ...
  products.forEach(product => {
    const gridCard = renderCard(product, 'grid');
    const listCard = renderCard(product, 'list');
    if (gridCard) gridContainer.appendChild(gridCard);
    if (listCard) listContainer.appendChild(listCard);
  });
  ...
  if (toggleView) {
    const gridIcon = img({ src: '/icons/grid.svg', alt: 'Grid View', id: 'grid-view-toggle', class: 'w-5 h-5 cursor-pointer opacity-100' });
    const listIcon = img({ src: '/icons/list.svg', alt: 'List View', id: 'list-view-toggle', class: 'w-5 h-5 cursor-pointer opacity-50' });
    toggleButtons = div({ class: 'flex items-center gap-2 ml-4' }, gridIcon, listIcon);
    controls.append(toggleButtons);
  }
  ...
  const scrollWrapper = div({ class: 'overflow-hidden w-full' }, gridContainer, listContainer);
  ...
  const scrollToIndex = (index) => {
    ...
    gridContainer.style.transform = `translateX(-${cardWidth * index}px)`;
    currentIndex = index;
    updateArrows();
  };
