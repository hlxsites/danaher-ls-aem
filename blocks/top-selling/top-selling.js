...
export default async function decorate(block) {
  ...
  products.forEach((product) => {
    ...
    const card = div({
      class: 'min-w-[23%] w-[23%] flex-shrink-0 bg-white border rounded-lg p-4 flex flex-col justify-between h-[430px]'
    });
...
export default async function decorate(block) {
  ...
  products.forEach((product) => {
    ...
    const card = div({
      class: 'min-w-[23%] w-[23%] flex-shrink-0 bg-white border rounded-lg p-4 flex flex-col justify-between h-[470px]'
    });

    if (image) card.append(img({ src: image, alt: title, class: 'w-full h-32 object-contain mb-4' }));
    if (title) card.append(p({ class: 'text-base font-semibold text-black mb-1 line-clamp-2' }, title));
