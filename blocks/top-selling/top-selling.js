...
export default function decorate(block) {
  ...
  const header = div({ class: 'w-full flex flex-col sm:flex-row justify-between items-center gap-3 mb-4' },
    div({ class: 'flex flex-wrap sm:flex-nowrap items-center gap-4' },
      div({ class: 'text-black text-2xl font-bold whitespace-nowrap' }, headingText),
    ...
    div({ class: 'w-72 inline-flex justify-end items-center gap-6' },
      div({ class: 'flex justify-start items-center gap-3' },
        button({ class: 'carousel-prev-div w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center', onclick: () => changeSlide(-1) },
          span({ class: 'icon icon-arrow-left w-5 h-5' })
        ),
        button({ class: 'carousel-next-div w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center', onclick: () => changeSlide(1) },
          span({ class: 'icon icon-arrow-right w-5 h-5' })
        )
      ),
      div({ class: 'flex justify-start items-center' },
        button({ class: 'toggle-view-list px-3 py-2 bg-white rounded-l-full outline outline-1 outline-violet-600', onclick: () => switchView(false) },
          span({ class: 'icon icon-view-list w-5 h-5 text-gray-600' })
        ),
        button({ class: 'toggle-view-grid px-3 py-2 bg-violet-600 text-white rounded-r-full outline outline-1 outline-violet-600', onclick: () => switchView(true) },
          span({ class: 'icon icon-view-grid w-5 h-5 text-white' })
        )
      )
    )
  );
  ...
  function renderCard(item) {
    ...
    return div({ class: 'w-full sm:w-[calc(50%-10px)] lg:w-[calc(25%-15px)] bg-white outline outline-1 outline-gray-300 flex flex-col' },
      ...
      description && p({ class: 'px-3 pb-3 text-gray-700 text-sm line-clamp-4' }, description),
      div({ class: 'self-stretch px-4 py-3 bg-gray-50 flex flex-col items-end gap-4' },
        div({ class: 'text-black text-2xl font-bold text-right' }, price),
        ...
        div({ class: 'flex gap-3 items-center mt-3 justify-end w-full' },
          buyText && button({ class: 'w-24 px-5 py-2 bg-violet-600 text-white rounded-full outline outline-1 outline-violet-600' }, buyText),
