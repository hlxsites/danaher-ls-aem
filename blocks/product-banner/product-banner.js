...
export default function productBannerDecorate(block) {
  ...
  const fieldsArray = Array.from(block.children).filter(
    (el) => el.tagName === "DIV"
  );
  ...
  const categoryHeading = fieldsArray[0].textContent || "";
  const btnText = fieldsArray[1].textContent || "";
  const btnLink = fieldsArray[2].textContent.trim() || "#";
  const rawCategoryDescription = fieldsArray[3].innerHTML || "";
  const details = fieldsArray[4].innerHTML || "";
