...
import { ul, li, a, p, div, span, h4 } from "../../scripts/dom-builder.js";
...
export function createCard(product, firstCard = false) {
  const cardWrapper = a(
    ...
    p(
      {
        class:
          "!px-7 mb-4 text-sm text-gray-900 break-words line-clamp-4 !h-20",
      },
      product.description
    ),
    div(
    ...
    .filter(({ fullCategory }) => {
      if (fullCategory) {
        ...
        if (categories.length > 1)
          return categories.at(1).toLowerCase() === category.toLowerCase();
      }
      return false;
    })
    .all();
