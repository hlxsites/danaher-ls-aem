import { a, div, li, nav, span, ul } from "../../scripts/dom-builder.js";
...
function createTabList(tabs, currentTab) {
  return ul({ class: 'flex justify-center overflow-hidden bg-white bg-opacity-0 rounded-lg shadow-lg' },
    ...tabs.map((tab) => {
      const isSelectedTab = tab.id === currentTab;
      const navItem = li({ class: 'flex items-center justify-center w-32 h-32 overflow-hidden capitalize bg-gray-50 group', 'data-tabid': tab.id, 'aria-selected': isSelectedTab },
        a({
      ...
      // navItem.querySelector('a .tab-icon').innerHTML = tabIcons[tab.id][selectedTab ? 'highlighted' : 'normal'];
