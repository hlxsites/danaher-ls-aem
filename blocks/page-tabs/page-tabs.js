...
export function createTabList(tabs, currentTab, isJumpMenu) {
  const ulTag = ul(
    ...
    ...tabs.map((tab) => {
      ...
      const navItem = li(
        {
          class: "overflow-hidden capitalize group",
          role: "tab",
          "data-tabid": tab.id,
        ...
        a(
          {
            class:
              "w-48 flex gap-x-2 pl-3 pr-2 bg-white text-danaherblack-500 group-hover:text-white group-hover:bg-danaherpurple-500",
            href: ancHref,
            title: tab.name,
          },
          span({
            class: `w-8 h-8 icon ${tabIcon} stroke-1 stroke-danaherpurple-500 group-hover:stroke-white`,
          }),
          span({ class: "py-2 text-sm tracking-wider font-bold" }, tab.name),
          span({
            class: `icon icon-chevron-down mt-4 mb-2 [&_svg]:duration-300 [&_svg]:stroke-1 [&_svg]:group-hover:translate-y-1 [&_svg]:group-hover:stroke-white ${
              isSelectedTab
                ? "[&_svg]:stroke-white"
                : "[&_svg]:stroke-danaherpurple-500"
            }`,
