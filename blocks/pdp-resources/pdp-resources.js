  ...
  resourceResultList.subscribe(() => {
    ...
    window.nextResults = () => {
      if (resourcePager.state.hasNextPage) {
        ...
        console.log("Next page called manually")
      } else {
        console.warn("🚫 No more pages")
      }
    }

    console.table(resourceResultList.state.results, ["title", "clickUri", "raw.opco", "raw.documenttype"])
    ...
    const renderPagination = () => {
      ...
      const currentPage = resourcePager.state.currentPage
      ...
      nextButton.append(
        ...
        div(
          ...
          div({ class: "justify-start text-sm font-medium leading-5" }, "Next"),
          div(
            { class: "w-5 h-5 relative overflow-hidden" },
            span({
