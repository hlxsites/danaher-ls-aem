...
export default async function decorate(block) {
  ...
  const accordionContainerTitle = fieldsArray[0].textContent.trim() || "";
  const customUUID = generateUUID();
  console.log("fieldsArray: ", fieldsArray);

  const acrItems = fieldsArray[1];

  const dynamicData = fieldsArray
    .map((element, index) => {
      if (index > 0 && index < fieldsArray.length) {
        const question = fieldsArray[index].textContent;
        const answer = fieldsArray[index + 1].textContent;
