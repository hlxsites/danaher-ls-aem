...
:::::::::::::::::
*/
    showAddressModal?.addEventListener('click', async (e) => {
      e.preventDefault();
      showPreLoader();
      /*
      ::::::::::::::
   ...
   */
    ...
    *
    ...
    if (
      ...
      const address = getDefaultAddressesResponse.data.filter((adr) => adr.preferredShippingAddress === 'true');
...
function generateDefaultAddress(
  ...
  if (getDefaultAddressesResponse?.status === 'success') {
    const address = getDefaultAddressesResponse?.data?.filter((adr) => adr.preferredBillingAddress === 'true');
